...
def get_item(value, type):
    ...
    subtitle = ''
    if (2 == type):
        bin = str(NumberUtils.dec_2_bin(value))
        title = 'bin : ' + bin
        subtitle = '二进制 : ' + bin
        return item(title=title, subtitle=subtitle, arg=bin)
    elif (8 == type):
        oct = str(NumberUtils.dec_2_oct(value))
        title = 'oct : ' + oct
        subtitle = '八进制 : ' + oct
        ...
        hex = str(NumberUtils.dec_2_hex(value))
        title = 'hex : ' + hex
        subtitle = '十六进制 : ' + hex
        return item(title=title, subtitle=subtitle, arg=hex)
    else:
        return item(title='十进制转换', subtitle='十进制转换', arg='')
...
def main(wf):
    ...
    try:
        if (value):
            ...
            wf.add_item(title='十进制转换', subtitle='十进制转换', arg='', valid=True)
    except:
        return

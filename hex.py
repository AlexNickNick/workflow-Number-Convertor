...
def get_item(value, type):
    ...
    subtitle = ''
    if (2 == type):
        bin = str(NumberUtils.hex_2_bin(value))
        title = 'bin : ' + bin
        subtitle = '二进制 : ' + bin
        return item(title=title, subtitle=subtitle, arg=bin)
    elif (8 == type):
        oct = str(NumberUtils.hex_2_oct(value))
        title = 'oct : ' + oct
        subtitle = '八进制 : ' + oct
        return item(title=title, subtitle=subtitle, arg=oct)
    elif (10 == type):
        dec = str(NumberUtils.hex_2_dec(value))
        title = 'dec : ' + dec
        subtitle = '十进制 : ' + dec
        ...
        return item(title='十六进制转换', subtitle='十六进制转换', arg='')
...
def main(wf):
    ...
    try:
        if (value):
            ...
            wf.add_item(title='十六进制转换', subtitle='十六进制转换', arg='', valid=True)
    except:
    	return
    wf.send_feedback()
